...
module.exports = class Router extends EventEmitter {
    constructor(sink, options = {}) {
        ...
        this.options = { env: NODE_ENV || DEFAULT_NODE_ENV, ...options };
        ...
        this.app.post('/publish-assets', async (req, res, next) => {
            try {
                ...
                const bundler = new OptimisticBundler({ sink: this.sink });
                const meta = await bundler.publishAssets(payload);
                res.send(meta);
            } catch (err) {
                next(err);
            }
        ...
        this.app.post('/publish-instructions', async (req, res, next) => {
            try {
                const payload = await parseBody(req, res);
                const bundler = new OptimisticBundler({
                    env: this.options.env,
                    minify:
                        req.query.minify === 'true' ||
                        this.options.isProduction,
                    sink: this.sink,
                    sourceMaps: req.query.sourceMaps === 'true',
                });
                await bundler.publishInstructions(payload);
                res.sendStatus(204);
            } catch (err) {
                next(err);
            }
