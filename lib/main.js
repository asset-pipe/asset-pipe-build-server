...
const publishAssets = require('./publish-assets');
const publishInstructions = require('./publish-instructions');
    ...
    constructor(sink, options = {}) {
        ...
        this.app.post('/publish-assets', async (req, res, next) => {
            try {
                ...
                const meta = await publishAssets(this.sink, payload);
                res.send(meta);
            } catch (err) {
                next(err);
            }
        ...
        this.app.post('/publish-instructions', async (req, res, next) => {
            try {
                const payload = await parseBody(req, res);
                await publishInstructions(this.sink, payload);
                res.status(204).end();
            } catch (err) {
                next(err);
            }
